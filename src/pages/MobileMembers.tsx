...
const ROLE_OPTIONS = ["Pastor", "Diácono", "Obreiro", "Líder de Célula", "Membro", "Visitante"];
const STATUS_OPTIONS = ["Ativo", "Inativo", "Licença", "Pendente", "Acompanhado", "Decidido", "Afastado"];
...
export default function MobileMembers() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      // defensive: ensure payload size is reasonable; remove photo if still too large
      try {
        const size = new TextEncoder().encode(JSON.stringify(payload)).length;
        if (size > 950 * 1024) {
          // remove photo and notify
      ...
      const existingMembers = localStorage.getItem("members_data");
      let membersList: any[] = [];
      try {
        membersList = existingMembers ? JSON.parse(existingMembers) : [];
      } catch {
        membersList = [];
      }
      cacheRecordsWithoutEmbeddedPhotos("members_data", [newMemberRecord, ...membersList]);
      setSuccessMessage("Cadastro realizado com sucesso!");

      setMessage("Cadastro enviado com sucesso! Ele aparecerá no sistema principal.");
      setName("");
      setRole("Membro");
      setPhone("");
      setEmail("");
      ...
      <div className="space-y-6">
        <div className="rounded-3xl border border-emerald-500/20 bg-emerald-500/10 p-6">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.3em] text-emerald-300">Primeiro passo</p>
              <h2 className="mt-2 text-2xl font-bold text-white">Tire sua foto</h2>
              <p className="mt-2 text-sm leading-6 text-zinc-300">A foto ajuda a identificar o membro no sistema pastoral.</p>
            </div>
            <Link to="/mobile" className="text-sm font-semibold text-emerald-300 hover:text-emerald-200">
              Voltar
            </Link>
          </div>
        </div>
        ...
        {message ? (
          <div className="rounded-3xl border border-white/10 bg-emerald-500/10 px-4 py-3 text-sm text-emerald-100">
            {message}
          </div>
        ) : null}

        <div className="rounded-[2rem] border border-white/10 bg-white/5 p-6 text-center">
          <div className="mx-auto grid h-48 w-48 place-items-center overflow-hidden rounded-full border border-emerald-500/25 bg-black/20">
            {photoPreview ? (
              <img src={photoPreview} alt="Preview da foto" className="h-full w-full object-cover" />
            ) : (
              <Camera size={64} className="text-emerald-300" />
            )}
          </div>

          <label className="mt-6 inline-flex w-full cursor-pointer items-center justify-center gap-3 rounded-3xl bg-emerald-500 px-5 py-4 text-sm font-bold text-white shadow-lg shadow-emerald-500/20 transition hover:bg-emerald-400">
            <ImagePlus size={18} />
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-3xl border border-white/10 bg-white/5 p-6">
        <div className="flex items-center justify-between gap-4">
          <div>
            <h2 className="text-xl font-semibold text-white">Novo Membro</h2>
            <p className="text-sm text-zinc-400">Preencha os dados e envie para o painel principal.</p>
          </div>
          <Link to="/mobile" className="text-sm text-emerald-300 hover:text-emerald-200">
            Voltar
          </Link>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="grid gap-4">
        ...
        <button
          type="submit"
          disabled={isSaving}
          className="rounded-3xl bg-emerald-500 px-4 py-3 text-sm font-semibold text-white transition hover:bg-emerald-400 disabled:cursor-not-allowed disabled:opacity-60"
        >
