...
import { createServerYouth } from "../services/crm.service";
...
export default function MobileYouth() {
  const navigate = useNavigate();
  const [name, setName] = useState("");
  const [role] = useState("Jovem");
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const newYouthRecord = { ...response.youth, createdByMobile: true };
      const existingYouth = localStorage.getItem("youth_data");
      let youthList: any[] = [];
      try {
        youthList = existingYouth ? JSON.parse(existingYouth) : [];
      } catch {
        youthList = [];
      }
      cacheRecordsWithoutEmbeddedPhotos("youth_data", [newYouthRecord, ...youthList]);
      ...
      <div className="space-y-6">
        <div className="rounded-3xl border border-orange-500/20 bg-orange-500/10 p-6">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.3em] text-orange-300">Primeiro passo</p>
              <h2 className="mt-2 text-2xl font-bold text-white">Tire sua foto</h2>
              <p className="mt-2 text-sm leading-6 text-zinc-300">A foto ajuda a identificar o jovem no sistema.</p>
            </div>
            <Link to="/mobile" className="text-sm font-semibold text-orange-300 hover:text-orange-200">
              Voltar
            </Link>
          </div>
        </div>
        ...
        {message ? (
          <div className="rounded-3xl border border-white/10 bg-orange-500/10 px-4 py-3 text-sm text-orange-100">
            {message}
          </div>
        ) : null}

        <div className="rounded-[2rem] border border-white/10 bg-white/5 p-6 text-center">
          <div className="mx-auto grid h-48 w-48 place-items-center overflow-hidden rounded-full border border-orange-500/25 bg-black/20">
            {photoPreview ? (
          ...
          <label className="mt-6 inline-flex w-full cursor-pointer items-center justify-center gap-3 rounded-3xl bg-orange-500 px-5 py-4 text-sm font-bold text-white shadow-lg shadow-orange-500/20 transition hover:bg-orange-400">
            <ImagePlus size={18} />
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-3xl border border-white/10 bg-white/5 p-6">
        <div className="flex items-center justify-between gap-4">
          <div>
            <h2 className="text-xl font-semibold text-white">Novo Jovem</h2>
            <p className="text-sm text-zinc-400">Registre um jovem no sistema mobile.</p>
          </div>
          <Link to="/mobile" className="text-sm text-orange-300 hover:text-orange-200">
            Voltar
          </Link>
        </div>
      </div>
      ...
      {message ? (
        <div className="rounded-3xl border border-orange-500/20 bg-orange-500/10 px-4 py-3 text-sm text-orange-100">
          {message}
        </div>
      ) : null}
      ...
      <form onSubmit={handleSubmit} className="grid gap-4">
        ...
        <div className="rounded-3xl border border-emerald-500/20 bg-emerald-500/10 px-4 py-3 text-xs text-emerald-100">
          Data de registro: automática no momento do cadastro.
        </div>
        <input
          value={referredBy}
        ...
        <button
          type="submit"
          disabled={isSaving}
          className="rounded-3xl bg-orange-500 px-4 py-3 text-sm font-semibold text-white transition hover:bg-orange-400 disabled:cursor-not-allowed disabled:opacity-60"
        >
